...
var Notification = React.createClass({
  ...
  errorHandler: function (notificationId, errorMessage, functionName) {
    if (this.props.onError) {
      if (typeof this.props.onError === 'function') {
        if (notificationId === 0) { errorMessage += ", notificationId doesn\'t have an ID!" }
        if (functionName) { errorMessage = 'in function: ' + functionName + ' ' + errorMessage }
  ...
  acceptHandler: function () {
    try {
      ...
      var id = 0
      if (this.props.data) {
        if (this.props.data.id) { id = this.props.data.id }
      }
