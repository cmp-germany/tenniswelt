  	    ...
  	    dataType: 'json'
  			...
  			if (result.length > 0) {
  				...
  				$.each(this.state.messageThreads, function (key, value) {
  					var userObject = $.grep(result, function (e) { return e.userId == value.ToUserId })
  ...
  setSeenUser: function (userId) {
    ...
    $.each(this.state.messageThreads, function (key, value) {
      if (value.ToUserId == userId) {
        if (!value.IsSeen) { this.state.unseenMessagesCount = this.state.unseenMessagesCount - value.CountOfUnreadMessages }
